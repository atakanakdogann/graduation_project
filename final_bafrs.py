...
import json
import os
import copy # Added to copy routes

# --- Parameters and other initial code remain the same ---
# ... (n, m, K, H, T, D, ah, Qj, ... and your coordinates are located here)
n = 333; m = 3; K = 1; H = 3; T = 4; D = 2.0
ah = [2, 3, 4]; Qj = [120, 240, 660]; uj = [0.5, 1.0, 2.5]
CPj = [100, 180, 400]; CTj = [10, 10, 10]; CRj = [10, 10, 10]
Mj = [200, 200, 100]; Wjk = [[1], [1], [1]]; vehicle_capacity = 0.3
...
def calculate_route_details(route, k):
    """Calculates the load and length of a route."""
    load = 0
    length = 0
    for i in range(len(route) - 1):
        ...
        if end_node < n: # If the node is a customer
            load += qi[end_node][k]
        elif end_node >= n + 1: # If the node is an IF
            load = 0 # Reset the load
    return load, length

# ---------------------- INTRA-ROUTE MOVES ----------------------
def two_opt(route):
    # This function calculates its own cost as it improves the route
    best_route = route
...
# ---------------------- INTER-ROUTE MOVES ----------------------
def inter_route_swap(solution, k):
    """
    Swaps one customer between two different routes in a daily solution.
    The swap is applied if it does not violate constraints (D and vehicle_capacity).
    """
    new_solution = copy.deepcopy(solution)
    # Select two different routes to swap between
    if len(new_solution) < 2: return None, None # Swap cannot be performed if there is only one route
    r1_idx, r2_idx = random.sample(range(len(new_solution)), 2)
    
    route1 = new_solution[r1_idx]
    route2 = new_solution[r2_idx]

    # Select customers to be swapped from the routes (must not be a depot or IF)
    custs1 = [node for node in route1[1:-1] if node < n]
    custs2 = [node for node in route2[1:-1] if node < n]
    if not custs1 or not custs2: return None, None
    ...
    # Check constraints
    _, len1 = calculate_route_details(route1, k)
    _, len2 = calculate_route_details(route2, k)

    if len1 > D or len2 > D:
        return None, None # Route length constraint violated

    # If capacity constraint also needs to be checked, it can be added here.
    # For simplicity, only length is checked in this example.

    return new_solution, solution_cost(new_solution)

def inter_route_relocate(solution, k):
    """
    Relocates a customer from one route to the best position in another route.
    The relocation is applied if it does not violate constraints.
    """
    new_solution = copy.deepcopy(solution)
    if len(new_solution) < 2: return None, None

    # Select the source and destination routes
    ...
    custs_from = [node for node in route_from[1:-1] if node < n]
    if not custs_from: return None, None

    node_to_move = random.choice(custs_from)
    
    # Remove the customer from their original route
    route_from.remove(node_to_move)

    # If the route becomes empty (only depot-depot remains), update the solution
    if len(route_from) <= 2:
    ...
    for i in range(1, len(route_to)):
        ...
        _, length = calculate_route_details(temp_route_to, k)
        if length <= D: # Only consider valid insertions
            cost_increase = d_ij[route_to[i-1]][node_to_move] + d_ij[node_to_move][route_to[i]] - d_ij[route_to[i-1]][route_to[i]]
    ...
    if best_insert_pos != -1:
        ...
        return None, None # No suitable insertion position found

# ---------------------- HOLISTIC VNS OPTIMIZATION FUNCTION ----------------------
def vns_for_a_days_plan(initial_solution, k, max_iter=200):
    """
    Optimizes the entire route plan for a single day as a whole.
    Uses both inter-route and intra-route moves.
    """
    best_solution = initial_solution
    best_cost = solution_cost(best_solution)
    
    inter_route_neighborhoods = [inter_route_swap, inter_route_relocate]
    
    print(f"     VNS Initial Cost: {best_cost:.4f}, Number of Routes: {len(best_solution)}")

    for i in range(max_iter):
        # Select a random inter-route neighborhood
        nh = random.choice(inter_route_neighborhoods)
        
        # Generate a new candidate solution
        candidate_solution, candidate_cost = nh(best_solution, k)

        if candidate_solution is None: # Invalid or unsuccessful move
            continue

        # If the inter-route move was successful, improve the affected routes with 2-opt.
        # This combines local search with VNS and is very effective.
        final_candidate_solution = copy.deepcopy(candidate_solution)
        for r_idx in range(len(final_candidate_solution)):
            final_candidate_solution[r_idx] = two_opt(final_candidate_solution[r_idx])
        
        candidate_cost = solution_cost(final_candidate_solution)

        # Accept if there is an improvement
        if candidate_cost < best_cost:
            best_solution = final_candidate_solution
            best_cost = candidate_cost
            print(f"     -> Improvement found! Iter {i+1}, New Cost: {best_cost:.4f}, Number of Routes: {len(best_solution)}")
...
def solve_pvrp_vns(routing_plan, D, max_iter=50):
    total_routing_cost = 0
    depot_idx = n
    IF_indices = list(range(n + 1, len(nodes)))
    
    detailed_routes_list = []
    final_route_counter = 0
    
    cost_multiplier = 10000

    for k in range(K): # Waste Type Loop
        for d in range(1, T + 1): # Day Loop
            unserved_customers = set(routing_plan[k][d])
            
            if not unserved_customers:
                print(f"   [Day {d}/{T}, Waste {k}] No sites to visit today. Skipping.")
                continue
            
            print(f"\n--- Day {d}, Waste {k} Planning Started. Number of Customers: {len(unserved_customers)} ---")

            # STEP 1: CREATE A VALID INITIAL SOLUTION FOR THAT DAY
            # This performs a simple greedy assignment, similar to our old method.
            # The result will be inefficient, but it is a starting point for VNS.
            initial_daily_routes = []
            temp_unserved = set(unserved_customers)
            
            while temp_unserved:
                new_route = [depot_idx]
                last_node_idx = depot_idx
                
                # Fill the route by adding the closest customers possible
                # (This part can be made smarter, but it is sufficient for an initial solution)
                sorted_customers = sorted(list(temp_unserved), key=lambda c: d_ij[last_node_idx][c])

                for customer in sorted_customers:
                    if customer not in temp_unserved: continue

                    # Check capacity and length constraints
                    _, current_len = calculate_route_details(new_route, k)
                    
                    # Can this customer be added to the route?
                    if current_len + d_ij[last_node_idx][customer] + d_ij[customer][depot_idx] <= D:
                        new_route.append(customer)
                        temp_unserved.remove(customer)
                        last_node_idx = customer
                
                new_route.append(depot_idx)
                initial_daily_routes.append(new_route)

            # STEP 2: OPTIMIZE THE CREATED DAILY PLAN WITH INTER-ROUTE VNS
            print(f"   Initial Plan Created. {len(initial_daily_routes)} vehicles will be used.")
            
            optimized_daily_routes = vns_for_a_days_plan(initial_daily_routes, k, max_iter=max_iter)

            # STEP 3: SAVE THE OPTIMIZED RESULTS
            for route_nodes in optimized_daily_routes:
                if len(route_nodes) <= 2: continue # Skip empty routes
                final_route_counter += 1
                
                _, route_len = calculate_route_details(route_nodes, k)
                
                route_info = {
                    "route_id": final_route_counter, "day": d, "waste_type": k,
                    "nodes": route_nodes, "optimized_cost": round(route_cost(route_nodes), 4),
                }
...
def solve_scenario4_cplex_with_fihk():
    mdl = Model("Scenario4_BAFRS_FIHK_Optimized"); xijk = mdl.integer_var_dict(((i, j, k) for i in range(n) for j in range(m) for k in range(K)), name="x"); fihk = mdl.binary_var_dict(((i, h, k) for i in range(n) for h in range(H) for k in range(K)), name="fihk"); z_plus_ij = mdl.continuous_var_dict(((i, j) for i in range(n) for j in range(m)), name="zplus"); z_minus_ij = mdl.continuous_var_dict(((i, j) for i in range(n) for j in range(m)), name="zminus"); wj = mdl.continuous_var_dict(range(m), name="w")
    mdl.minimize(mdl.sum(CPj[j] * wj[j] for j in range(m)) + mdl.sum(CRj[j] * z_minus_ij[i, j] for i in range(n) for j in range(m)) + mdl.sum(CTj[j] * z_plus_ij[i, j] for i in range(n) for j in range(m)))
    for i in range(n):
        for k in range(K):
            mdl.add_constraint(mdl.sum(Qj[j] * xijk[i, j, k] * Wjk[j][k] for j in range(m)) >= qi[i][k] * mdl.sum(ah[h] * fihk[i, h, k] for h in range(H))); mdl.add_constraint(mdl.sum(fihk[i, h, k] for h in range(H)) == 1)
    for i in range(n): mdl.add_constraint(mdl.sum(xijk[i, j, k] * uj[j] for j in range(m) for k in range(K)) <= Ui[i])
    for j in range(m): mdl.add_constraint(wj[j] <= Mj[j])
    for i in range(n):
        for j in range(m):
            mdl.add_constraint(z_plus_ij[i, j] >= mdl.sum(xijk[i, j, k] for k in range(K)) - pij[i][j]); mdl.add_constraint(z_minus_ij[i, j] >= pij[i][j] - mdl.sum(xijk[i, j, k] for k in range(K))); mdl.add_constraint(mdl.sum(xijk[i, j, k] for k in range(K)) - z_plus_ij[i, j] + z_minus_ij[i, j] == pij[i][j])
    for j in range(m): mdl.add_constraint(wj[j] >= mdl.sum(z_plus_ij[i, j] - z_minus_ij[i, j] for i in range(n)))
    sol = mdl.solve(log_output=False)
    if sol: return sol, fihk, wj, z_plus_ij, z_minus_ij
    else: return None, None, None, None, None

def extract_routing_plan(sol, fihk_vars):
    plan = {k: {d: [] for d in range(1, T + 1)} for k in range(K)}
    for i in range(n):
        for k in range(K):
            for h in range(H):
                if sol.get_value(fihk_vars[i, h, k]) > 0.5:
                    freq_value = ah[h]; num_visits = T // freq_value if freq_value > 0 else T
                    days = sorted(random.sample(range(1, T + 1), min(T, max(1, num_visits))))
                    for d in days: plan[k][d].append(i)
    return plan
...
def run_bafrs_total_for_analysis():
    print("--- STARTING: Integrated Container and Route Optimization (Inter-Route VNS) ---")
    total_start_time = time.time()
    
    print("\n--- STAGE 1: Strategic Planning (MILP Solution) ---")
    milp_start_time = time.time()
    solution, fihk_vars, wj_vars, z_plus_vars, z_minus_vars = solve_scenario4_cplex_with_fihk()
    milp_duration = time.time() - milp_start_time
    if solution is None:
        print("ERROR: MILP problem could not be solved. Analysis stopped."); return
    print(f"MILP solution completed in {milp_duration:.2f} seconds.")
    bin_cost = solution.objective_value
    milp_results = { "solution_time_sec": round(milp_duration, 2), "total_container_cost": round(bin_cost, 2), #...
    } 
    routing_plan = extract_routing_plan(solution, fihk_vars)

    print("\n--- STAGE 2: Operational Routing (Enhanced Inter-Route VNS) ---")
    print("Optimizing daily routes with VNS...")
    
    routing_cost, detailed_routes = solve_pvrp_vns(routing_plan, D, max_iter=200) # Increasing the number of iterations can be beneficial
    print("Routing completed.")

    vns_results = {
        "total_routing_cost": round(routing_cost, 2),
        "total_route_count": len(detailed_routes),
        "routes": detailed_routes
    }
    
    total_cost = bin_cost + routing_cost
    ...
    final_analysis_data = {
        "analysis_results": {
            ...
            "total_execution_time_sec": round(total_duration, 2),
            "milp_solution": milp_results,
            "vns_routing": vns_results
        },
    ...
    print(json.dumps(final_analysis_data, indent=2, ensure_ascii=False))
    print("--- END OF JSON OUTPUT ---\n")

    # --- Save results to a JSON file ---
    try:
        with open("bafrs_results.json", "w", encoding="utf-8") as f:
            json.dump(final_analysis_data, f, ensure_ascii=False, indent=4)
        print("Analysis results successfully saved to 'bafrs_results.json'.")
    except Exception as e:
        print(f"ERROR: Results could not be saved to JSON file. Error: {e}")

    print("\n--- STAGE 4: Visualizing and Saving Daily Routes ---")
    plot_routes_by_day(
        coordinates=coordinates, depot=depot, IFs=IFs, 
        detailed_routes=vns_results["routes"], 
        output_folder="daily_route_visuals_v2"
    )

# ---------------------- Run The Analysis ----------------------
if __name__ == "__main__":
    run_bafrs_total_for_analysis()